...
// SetMap sets a column to a given value. If `Set` was called before with
// the same column name, it overrides the value of the previous call.
...
func (u *UpdateBuilder) Save(ctx context.Context) (sql.Result, error) {
	for _, iter := range u.driver.beforeUpdate {
		iter(u)
	}
	...
	switch {
	case u.driver.tx != nil:
		res, err = u.driver.tx.ExecContext(ctx, statement, args...)
	default:
		res, err = u.driver.driver.ExecContext(ctx, statement, args...)
	}

	for _, iter := range u.driver.afterUpdate {
		iter(u, res)
	}
...
func (d *DeleteBuilder) Exec(ctx context.Context) (sql.Result, error) {
	...
	switch {
	case d.driver.tx != nil:
		res, err = d.driver.tx.ExecContext(ctx, statement, args...)
	default:
		res, err = d.driver.driver.ExecContext(ctx, statement, args...)
	}

	for _, iter := range d.driver.afterDelete {
		iter(d, res)
	}
...
func Between(col string, v1, v2 any) *Predicate {
	return P().BetweenAnd(col, v1, v2)
	// return And(GTE(col, v1), LTE(col, v2))
}
...
func (s *Selector) Scan(ctx context.Context, dest any) error {
	...
	for _, iter := range s.driver.beforeQuery {
		iter(s)
	}
	...
	if s.driver.tx != nil {
		rows, err = s.driver.tx.QueryContext(ctx, statement, args...)
	} else {
		rows, err = s.driver.driver.QueryContext(ctx, statement, args...)
	}
	if err != nil {
		return err
	}
...
func Select(driver *DB, columns ...string) *Selector {
	return (&Selector{
		driver: driver,
	}).Select(columns...)
}
...
func (s *Selector) From(t string) *Selector {
	s.from = nil
	return s.AppendFrom(Table(t))
}

// FromTable sets the source of `FROM` clause.
...
// WhereMap sets or appends the given predicate to the statement.
func (s *Selector) WhereMap(p map[string]any) *Selector {

	args := make([]*Predicate, 0, len(p))
	for key, v := range p {
		args = append(args, EQ(key, v))
	}
...
func (b *Builder) WriteOp(op Op) *Builder {
	switch {
	case op >= OpEQ && op <= OpLike || op >= OpAdd && op <= OpBetweenAnd:
